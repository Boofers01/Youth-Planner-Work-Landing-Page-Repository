...
| `Images/photos/Backgrounds/` | `Hero Background.jpg` — the hero and auth-page background photo (was `hero-background-source.jpg`; filename now has a space, so CSS references it URL-encoded as `Hero%20Background.jpg`). |
| `Images/photos/Elements/` | `KeyboardVis Hero Devices Display.png` — the laptop + phones mockup graphic in the hero (this has been swapped a few times — `hero-iphone-mockup.png` → `Hero Phone Display.png` → `Overview Hero Phone Display.png` → `White Phone Hero Display.png` → `Hero Phone Display.png` → `Hero Devices Display.png` → `Textvis Hero Devices Display.png` → `KeyboardVis Hero Devices Display.png`; only the current filename exists on disk). Filename has spaces, so `index.html` references it URL-encoded (`KeyboardVis%20Hero%20Devices%20Display.png`). |
